...
int main(int argc, const char **argv) {
    ...
    arma::vec stressK22 = arma::zeros(n_bin);
	if (flag_stressK22 == 1) {
...
//                    }
					...
					if ( r_sq <= Force_cut_sq && (  type(i) != type_wall || type(j) != type_wall  ) ) {
						...
						for (int i_bin = 0; i_bin < n_bin; i_bin++) {
							...
							if (y_i > y_bin - R_cut - Force_cut && y_j < y_bin + R_cut + Force_cut && y_i> y_bin - R_cut - Force_cut && y_i < y_bin + R_cut + Force_cut) {
								...
								if (flag_stressV11 == 1) {
									stressV11(i_bin) += 1 / (Lx * Lz) * distx * (force * distx / r) * bond_function / n_steps;
								}
								if (flag_stressV12 == 1) {
									stressV12(i_bin) += 1 / (Lx * Lz) * distx * (force * disty / r) * bond_function / n_steps; //ok
								}
								if (flag_stressV13 == 1) {
									stressV13(i_bin) += 1 / (Lx * Lz) * distx * (force * distz / r) * bond_function / n_steps;
								}
								if (flag_stressV22 == 1) {
									stressV22(i_bin) += 1 / (Lx * Lz) * disty * (force * disty / r) * bond_function / n_steps; //ok
								}
								if (flag_stressV23 == 1) {
									stressV23(i_bin) += 1 / (Lx * Lz) * disty * (force * distz / r) * bond_function / n_steps;
								}
								if (flag_stressV33 == 1) {
									stressV33(i_bin) += 1 / (Lx * Lz) * distz * (force * distz / r) * bond_function / n_steps;
								}
							}//end if
						}//end for
					}
    ...
    //Compute local wall potential energy

    if (wall_potential_energy == 1) {
        std::cout << "Computing local wall potential energy \n";
        
        arma::vec wall_energy = arma::zeros(n_bin);
        ...
        for (size_t i_step = 0; i_step < n_steps; i_step++) {
            ...
            for (size_t i = 1; i < n_particles; i++) {
                ...
                for (size_t j = 0; j < n_particles ; j++) {
                    ...
                    double r_sq = distx*distx + disty*disty + distz*distz;
//                    && (  type(i) != 1 || type(j) != 1  ) && (  type(i) != 2 || type(j) != 2  )
                    if ( r_sq > 0 && r_sq <= Force_cut_sq  ) {
                        double r = std::sqrt(r_sq);
                        double r6i = 1.0 / (r_sq*r_sq*r_sq);
                        //double force = epsilon(type(i,i_step), type(j,i_step)) *r6i * (48.0 * r6i - 24.0) / r;
                        //pair_energy += 4.0 * r6i * (r6i - 1.0) / n_particles ;
                        //&& y_j > y_bin - Force_cut && y_j < y_bin + Force_cut
                        for (int i_bin = 0; i_bin < n_bin; i_bin++) {
                            double y_bin = bin_vec(i_bin);
                            if ( y_i > y_bin - R_cut && y_i < y_bin + R_cut  ) {
                            
                                wall_energy(i_bin) += 1 / (2*4* Lx * Lz * R_cut ) * 4.0 * epsilon( type(i,i_step), type(j,i_step)) *  r6i * (r6i - 1.0) / n_steps;
//                                wall_energy(i_bin) += 1 / (Lx * Lz) * 4.0 * epsilon( type(i,i_step), type(j,i_step)) *  r6i * (r6i - 1.0) * std::exp( -(y_i - y_bin)*(y_i - y_bin) / (2 * R_cut*R_cut)) / sqrt(2 * arma::datum::pi *R_cut*R_cut) / n_steps;
                            }//end if
                        }//end for
                    }
                } //end for j
        ...
        wall_energy.save("wall_energy.txt", arma::arma_ascii);
    } //end if
...
//            if (flag_kernel == 0 || flag_kernel == 2) {
    ...
    //Compute BULK (or VOLUME) viscosity
    if (flag_viscosity_bulk == 1) {
        std::cout << "Computing viscosity_bulk \n";
        arma::vec velocity_div = arma::zeros(n_bin);
        arma::vec viscosity_bulk = arma::zeros(n_bin);
        arma::vec density_derivative = arma::zeros(n_bin);
        arma::vec momentum_y_derivative = arma::zeros(n_bin);
        
        //if (flag_kernel == 0 || flag_kernel == 2) {
        if (flag_kernel == 0) {
            for (size_t i_bin = 1; i_bin < n_bin - 1; i_bin++) {
                velocity_div(i_bin) = ((momentum_y(i_bin + 1) - momentum_y(i_bin - 1)) * density(i_bin) - (density(i_bin + 1) - density(i_bin - 1)) * momentum_y(i_bin)) / ((bin_vec(i_bin + 1)-bin_vec(i_bin - 1))* density(i_bin)*density(i_bin));
                if (velocity_div(i_bin) != 0.0) {
                    viscosity_bulk(i_bin) = (stressK22(i_bin) + stressV22(i_bin)) / velocity_div(i_bin);
                }
                
                density_derivative(i_bin) = (density(i_bin + 1) - density(i_bin - 1))/ (bin_vec(i_bin + 1)-bin_vec(i_bin - 1));
                momentum_y_derivative(i_bin) = (momentum_y(i_bin + 1) - momentum_y(i_bin - 1))/ (bin_vec(i_bin + 1)-bin_vec(i_bin - 1));
            }
            density_derivative.save("density_derivative.txt", arma::arma_ascii);
            momentum_y_derivative.save("momentum_y_derivative.txt", arma::arma_ascii);
        }else{
            compute_density_derivative(n_steps, n_bin, Lx, Lz, R_cut, Mass, density_derivative, y, bin_vec, flag_kernel);
            
            compute_momentum_y_derivative(n_steps, n_bin, Lx, Lz, R_cut, vy, momentum_y_derivative, y, bin_vec, flag_kernel);
            
            velocity_div = (momentum_y_derivative - velocity_y % density_derivative) / density;
            
            viscosity_bulk= (stressK22+stressV22) / velocity_div;
            
            density_derivative.save("density_y_derivative.txt", arma::arma_ascii);
            momentum_y_derivative.save("momentum_y_derivative.txt", arma::arma_ascii);
        }
        
        viscosity_bulk.save("viscosity_bulk.txt", arma::arma_ascii);
        velocity_div.save("velocity_div.txt", arma::arma_ascii);
    }
